...
export default function ProgressBar({ id, activeCards, connection }) {
    ...
    return (
        ...
        <div id={id} className= {`progressBarContainer ${id === 'mobileProgressBar' ? 'mobileProgressBar' : 'desktopProgressBar'}`}>
            ...
            <div className="progressBar" style={{ width: `${100/6}%`, height: '100%', background: (activeCards[2] || activeCards[7])? 'rgba(57, 149, 230, 0.3)' : 'transparent' }} />
            ...
            <div className="progressBar" style={{ width: `${100/6}%`, height: '100%', background: (activeCards[5] || activeCards[6]) ? 'rgba(57, 149, 230, 0.3)' : 'transparent', borderRadius: '0 8px 8px 0' }} />
        </div>
